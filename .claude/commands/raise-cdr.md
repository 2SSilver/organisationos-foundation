...
5. Save as `<current-domain>/_drafts/cdr-{timestamp}-{slug}.md` — the current domain's own `_drafts/` folder, matching where `update-wiki` already drafts CDRs. Do not write into `../organisationos-foundation/`: a Domain session's `settings.json` denies writes there by design (the external-work boundary — Foundation CLAUDE.md → Confidentiality).
...
The CDR is reviewed and gated by the Leadership Forum. Below ~20 people, use `cdr-light-template.md` instead — every decision is an ADR with an `affected-domains` field.
